...
export function DeliveryOptions({ onSelectionChange }: DeliveryOptionsProps) {
  const { items } = useCartStore()
  const { formData, setShippingCost } = useCheckoutStore()
  ...
  const fetchDeliveryOptions = async () => {
    if (!formData.zipCode || items.length === 0) {
      return
    }
    ...
    try {
      ...
      const data: DeliveryOptionsResponse = await fetchAPI('/checkout/delivery-options', {
        ...
        body: JSON.stringify({
          destination_zip_code: formData.zipCode.replace(/\D/g, ''),
          items: orderItems
        })
      })
    ...
    return (
      <div className="space-y-4">
        ...
        <p className="text-sm text-muted-foreground">
          Preencha o CEP no formulário acima para ver as opções de entrega disponíveis.
        </p>
      </div>
    )
  }
